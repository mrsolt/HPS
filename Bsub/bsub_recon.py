...
def main() : 
    ...
    if (args.NLayers == "6"):
        ...
        if (args.Year == "2015"):
            ...
            SteeringFile = "EngineeringRun2015FullReconMC.lcsim" 

        if (args.Year == "2016"):
            Detector = "HPS-PhysicsRun2016-Nominal-v5-0-fieldmap"
            SteeringFile = "PhysicsRun2016FullRecon.lcsim"
    ...
    if (args.NLayers == "7"):
        ...
        if (args.Year == "2015"):
            ...
            SteeringFile = "EngineeringRun2015FullReconMCLayer0.lcsim"

        if (args.Year == "2016"):
            Detector = "HPS-Proposal2017-Nominal-v2-2pt3-fieldmap"
            SteeringFile = "PhysicsRun2016FullReconLayer0.lcsim"
    ...
    for line in file_list : 
        ...
        # Output file name
        output_path = line.strip().replace('_readout.slcio', '_recon')
        output_path = output_path[output_path.rfind("/")+1:]
        print "Writing readout file: " + output_path

        # Name and path of log file
        log_path = output_path.replace('_recon', '.log')
        print "Writing log to: " + log_path

        # Command that will be submitted to the batch system
        command = "java -jar " + args.JarFile + " -i " + line.strip() + " -Ddetector=" + Detector + " -Drun=" + Run + " -DoutputFile=" + output_path + " -r /org/hps/steering/recon/" + SteeringFile + " -n 100"
