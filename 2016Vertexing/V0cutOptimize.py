...
def saveCutFlow(eventsData,eventsAp,eventstruth,cuts,cutsdata,sigmaarr,varcutdata,varcutMC,nBins,minX,maxX,outfile,canvas,XaxisTitle="",YaxisTitle="",plotTitle="",stats=0,logY=0):
	outfileroot.cd()
	...
	ZBi = array.array('d')
	norm = 1
	for i in range(len(varcutdata)):
		zeroArr.append(0)
		cutstot = "{0}&&{1}".format(cuts,varcutMC[i])
		cutstotdata = "{0}&&{1}".format(cutsdata,varcutdata[i])
		...
		if(i == 0):
			norm = sigyield/back
		print("Signal Yield: {0}   Background: {1}   Sig/Back: {2}   Sigma: {3}".format(sigyield/norm,back,sigyield/(norm*back),sigmaarr[i]))
		...
		ZBi.append(Z_Bi)
		print("N on: {0}   N off: {1}   tau: {2}   ZBi: {3}   Sigma: {4}".format(n_on,n_off,tau,Z_Bi,sigmaarr[i]))
		del histo
		del histo2

	graph = TGraphErrors(len(varcutdata),sigmaarr,sigarr,zeroArr,sigErr)
	...
	graph2 = TGraphErrors(len(varcutdata),sigmaarr,ZBi,zeroArr,zeroArr)
...
varcutMC = []
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,1.0))
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,1.5))
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,2.0))
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,2.5))
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,3.0))
varcutMC.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,3.5))
...
varcutdata = []
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,1.0))
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,1.5))
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,2.0))
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,2.5))
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,3.0))
varcutdata.append("sqrt((({4}-{0})/({6}*{1}))^2+(({5}-{2})/({6}*{3}))^2)<1".format(uncTargProjX,uncTargProjXSig,uncTargProjY,uncTargProjYSig,xProj_rot,yProj_rot,3.5))

sigmaarr = array.array('d')
sigmaarr.append(1.0)
sigmaarr.append(1.5)
sigmaarr.append(2.0)
sigmaarr.append(2.5)
...
saveCutFlow(eventsData,eventsAp,eventstruth,nomcut,nomcutdata,sigmaarr,varcutdata,varcutMC,nBins,minVZ,maxVZ,outfile,c,XaxisTitle="V0 Projection N#sigma",YaxisTitle="Relative Signal / Background",plotTitle="Relative Signal / Background {0}".format(Label))
